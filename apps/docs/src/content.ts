...
<section id="pure-html">
    ...
    <p>No framework, no build step, no config object? Author the menu as plain markup and
    call <code>new Navalone()</code> with <strong>no <code>items</code></strong>. The core parses
    your <code>.menu-level</code> markup into the very same model the config produces, then
    renders the desktop bar and the mobile drawer from it — so a static HTML/CSS/JS page gets the
    identical menu the framework wrappers do.</p>
...
&lt;/script&gt;</code></pre>
    ...
    <em>Back</em> buttons, ARIA wiring and animations — is generated for you. You only author the
    structure below; the source classes you don't see in this table (wrappers, headers) are
    rebuilt, so there's nothing extra to keep in sync.</p>
