...
const optionGroups: { id: string; label: string; rows: string }[] = [
    ...
    },
    {
        ...
        rows: /* html */ `
            ...
            <tr><td><code>responsive</code></td><td>"dynamic" | "static"</td><td><code>"dynamic"</code></td><td>How collapsing is decided. <code>"dynamic"</code> (default) measures the center menu and folds it exactly when it would overlap the logo/buttons — regardless of screen size: it first <em>condenses</em> (smaller font, tighter spacing), then <em>collapses</em> to the drawer. <code>"static"</code> uses the fixed <code>breakpoint</code> (and optional <code>condenseBreakpoint</code>) pixel thresholds instead.</td></tr>
            <tr><td><code>breakpoint</code></td><td>number</td><td><code>960</code></td><td>Static mode only: max width (px) at which the bar collapses to the drawer. Ignored when <code>responsive: "dynamic"</code>.</td></tr>
            <tr><td><code>condenseBreakpoint</code></td><td>number | null</td><td><code>null</code></td><td>Static mode only: width (px) at or below which the bar condenses (but hasn't yet collapsed) — set it above <code>breakpoint</code>. <code>null</code> skips the condense step. Ignored when <code>responsive: "dynamic"</code> (which condenses automatically).</td></tr>
