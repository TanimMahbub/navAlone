/** Screenshot the built landing site: hero (top) and the code showcase. */
...
const profile = mkdtempSync(join(tmpdir(), "nv-shot-"));
const child = spawn(CHROME, ["--headless=new","--disable-gpu","--no-sandbox","--no-first-run","--force-prefers-reduced-motion","--hide-scrollbars",`--user-data-dir=${profile}`,`--remote-debugging-port=${dbg}`,"--window-size=1280,2400","about:blank"], { stdio: "ignore" });
...
try {
    ...
    // Full-page hero + code shot.
    let shot = await send("Page.captureScreenshot",{format:"png",captureBeyondViewport:true,clip:{x:0,y:0,width:1280,height:1500,scale:1}});
    writeFileSync(resolve(here,".phase4-shots","site-hero-new.png"), Buffer.from(shot.data,"base64"));

    // Scroll to code showcase and shot.
    await evalJS(send, `document.getElementById("get-started")?.scrollIntoView()`);
    await sleep(800);
    shot = await send("Page.captureScreenshot",{format:"png"});
    writeFileSync(resolve(here,".phase4-shots","site-code.png"), Buffer.from(shot.data,"base64"));
    console.log("shots written: site-hero-new.png, site-code.png");
    ws.close();
} finally {
